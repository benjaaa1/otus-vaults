...
contract OtusVault is BaseVault {
     ...
     */
    function trade(StrategyBase.StrikeTrade[] memory _strikes)
        external
        onlyOwner
        returns (uint[] memory positionIds)
    {
        ...
        for (uint i = 0; i < len; i++) {
            ...
            (positionId, premiumReceived, capitalUsed) = IStrategy(strategy)
                .doTrade(_trade);
    ...
    function reducePosition(uint positionId, uint closeAmount)
        external
        onlyKeeper
    {
  ...
  //  */
    ...
    function closeHedge(uint optionType) external onlyKeeper {
        require(
            activeHedgeByOptionType[optionType],
            "Vault has no active hedge for option type"
        );
    ...
    function withdrawSUSDSNX() public {
        uint balance = IERC20(0xaA5068dC2B3AADE533d3e52C6eeaadC6a8154c57)
