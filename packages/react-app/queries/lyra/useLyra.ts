...
const parseMarketResponse = async (markets: Market[]): Promise<LyraMarket[]> => {
	return await Promise.all(markets.map(async market => {
		...
		const liveBoardsWithQuotedStrikes: LyraBoard[] = await parseBoardStrikes(liveBoards); 
		console.log({ liveBoardsWithQuotedStrikes })
		return { address, name, isPaused, liveBoards: liveBoardsWithQuotedStrikes }
	}));
}
...
const parseMarketBoards = (boards: Board[]): LyraBoard[] => {
	return boards.map(board => {
		...
		const strikes: Strike[] = board.strikes().filter(strike => strike.isDeltaInRange); 
...
const parseBoardStrikes = async (boards: LyraBoard[]) => {
	return await Promise.all(boards.map(async board => {
		...
		const strikesLongCallQuotes = await formatStrikeWithQuote(strikes, true, true);
		const strikesLongPutQuotes = await formatStrikeWithQuote(strikes, false, true);
		const strikesShortCallQuotes = await formatStrikeWithQuote(strikes, true, false);
		const strikesShortPutQuotes = await formatStrikeWithQuote(strikes, false, false);

		return { ...board, strikesByOptionTypes: { 0: strikesLongCallQuotes, 1: strikesLongPutQuotes, 3: strikesShortCallQuotes, 4: strikesShortPutQuotes} }; 
	}));
}

const formatStrikeWithQuote = async (strikes: Strike[], isCall: boolean, isLong: boolean) => {
	return await Promise.all(strikes.map(async (strike: Strike) => {
