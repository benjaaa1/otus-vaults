...
const reportError = (
  ...
  const args: CreateToastOptions = {
    ...
    href: transactionReceipt
      ? getOptimismExplorerUrl(transactionReceipt.transactionHash)
      : undefined,
    autoClose: false,
  }
  if (toastId) {
    ...
    async (
      ...
      const toastId = !skipToast
        ? createPendingToast({
            ...
            } Confirm ${description} in ${getNameForWalletType(walletType)}`,
            autoClose: false,
          })
        : null
      ...
      if (toastId) {
        updatePendingToast(toastId, {
          ...
          } Your ${description} is pending, view on etherscan`,
          href: txHref,
          autoClose,
        })
      }
